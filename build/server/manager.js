...
function http_return(res, ret) {
    const httpReturn = { code: "0", msg: "success" };
...
// 服务注册
app.post(api_1.SERVER_REQUEST.CREATE, (req, res) => {
    const server_info = req.body;
    logger.debug(server_info);
    serverMgr_1.create_server_info(server_info);
    http_return(res, {});
});
// 服务查询
app.post(api_1.SERVER_REQUEST.FIND, (req, res) => {
    const find_info = req.body;
    http_return(res, { data: serverMgr_1.get_server_info(find_info.server_type, find_info.server_id) });
});
// 服务监控
app.get(api_1.SERVER_REQUEST.ALL, (req, res) => {
